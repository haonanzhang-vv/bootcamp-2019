...
sort(table(gapminder07$continent),decreasing = TRUE)

# Print out the population of Mexico in 2007
subset(gapminder,subset = (year == 2007) & (country == "Mexico"))

gapminder07_mexico <- subset(gapminder07,subset = country =="Mexico")
...
head(gapminder07$lifeExp_rounded)
...
reg1<-lm(lifeExp ~ gdpPercap + pop, data = gapminder)
...
write.csv(gapminder07,file = "data/gapminder_2007_edited.csv",row.names = FALSE)



#### Save R objects ####
save(schooldata,gapminder,file="workshopobjects.RData")
...
hist(gapminder07$lifeExp,breaks = 15, main = "Life expectancy 2007",ylab = "Frequency",xlab = "Life Expectancy")


# Bonus: Change the `breaks = ` argument from its default setting and see what happens.

#when the breaks become larger, the data pattern as well as noise are more obvious.
#### Scatterplots ####

# Create a scatterplot with `lifeExp` on the y-axis and `gdpPercap` on the x-axis.

plot(gapminder07$lifeExp ~ gapminder07$gdpPercap,main = "Life expectancy ~ GDP per capita (2007 data)", ylab = "LifeExp",xlab ="GDPpercap")
# Add a title and axis labels.


# Bonus: Add a horizontal line indicating the mean of `lifeExp` onto the plot using `abline()`.

abline(h=mean(gapminder07$lifeExp),v=mean(gapminder07$gdpPercap))
